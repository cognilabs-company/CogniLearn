...
def send_login_and_password_template_dashboard(user_email, user_password):
    email = EmailMessage()
    email['Subject'] = 'Password Reset Verification Code'
    email['From'] = MAIL_USERNAME
    email['To'] = user_email
...
@celery.task
def send_mail_login_password(email: str, password: str):
    try:
        email_msg = send_login_and_password_template_dashboard(email, password)
        with smtplib.SMTP_SSL(MAIL_SERVER, MAIL_PORT) as server:
            server.login(SMTP_USER, SMTP_PASSWORD)
            server.send_message(email_msg)
        print(f"Verification email sent to {email}")
