...
async def get_all(user: user_dependancy, db: db_dependency):
    ...
    if user_role.role_name == "admin" or user_role.role_name == "super admin" or user_role.role_name == "stuff":
...
async def get_by_status(user: user_dependancy, db: db_dependency, student_status: str):
    ...
    if user_role.role_name == "stuff":
        get_status = db.query(CallingProcess).filter(CallingProcess.status == student_status).all()
        return get_status
